...
export default function ExpenseList({ selectedDate }: Props) {
  const { expenses, removeExpense, updateExpense } = useExpenses()
  const { currency, exchangeRate } = useCurrency()

  // Modal States
  const [selectedExpense, setSelectedExpense] = useState<Expense | null>(null) // For Action Modal
  const [editingExpense, setEditingExpense] = useState<Expense | null>(null)   // For Edit Modal
  const [deleteId, setDeleteId] = useState<string | null>(null)                // For Delete Confirm
  ...
  return (
    <ListContainer>
        ...
        return (
          <GroupContainer key={date}>
            ...
            {groupedExpenses[date].map((exp) => {
              ...
              return (
                <ExpenseItem key={exp.id} onClick={(e) => handleItemClick(exp, e)}>
                  <LeftSection>
                    <IconCircle>
      ...
      {/* Action Modal (Edit/Delete/Cancel) */}
      <ExpenseActionModal
